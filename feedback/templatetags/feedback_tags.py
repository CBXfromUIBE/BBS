...
@register.simple_tag
def get_dislike_count(obj):
    content_type = ContentType.objects.get_for_model(obj)
    dislike_count,created = disLikeCount.objects.get_or_create(content_type=content_type,object_id=obj.pk)
    return dislike_count.disliked_num

@register.simple_tag(takes_context=True)
def get_dislike_status(context,obj):
    content_type = ContentType.objects.get_for_model(obj)
    user = context['user']
    if not user.is_authenticated:
        return ''
    if disLikeRecord.objects.filter(content_type=content_type,object_id=obj.pk,user=user).exists():
        return 'active'
    else:
        return ''
